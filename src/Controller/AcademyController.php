...
class AcademyController extends AbstractController
{
    ...
    {
        $user = $application->getUser();
        $html = $this->renderView('academy/pdfnew.html.twig', [
            'application' => $application,
            'userId' => $user->getId(),
        ]); 
    ...
    #[Route('/{id}/applications', name: 'app_academy_applications', methods: ['GET'])]
    public function applications(Request $request,Academy $academy, ApplicationRepository $apprepo, PaginatorInterface $paginator)
    {
        $qb = $apprepo->createQueryBuilder('c')
        ...
        $applications = $apprepo->findBy(['academy' => $academy]);

        return $this->render('academy/apps.html.twig', [
