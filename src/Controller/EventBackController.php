...
class EventBackController extends AbstractController
{
    ...
    {
        $stats = $eventRepository->getParticipantsStats();

        $data = [];
        $data[] = ['Event', 'Number of Participants'];

        foreach ($stats as $eventStats) {
            $data[] = [$eventStats['nom'], $eventStats['participantsCount']];
        }

        return $this->render('event_back/stats.html.twig', [
    ...
    public function search(EventRepository $eventRepository, Request $request): JsonResponse
    {
        $query = $request->query->get('q');

        $results = [];
        if ($query !== null ) {
            $results = $eventRepository->createQueryBuilderForSearch($query)->getQuery()->getResult();
        }

        $response = [];
        foreach ($results as $result) {
            $response[] = [
                'url' => $this->generateUrl('app_event_show', ['id' => $result->getId()]),
                'nom' => $result->getNom(),
            ];
        }
